  ...
  for (;;) {
    ...
    if (read(js->fd, &e, sizeof(e)) < 0) {
      ...
      if (errno == ENODEV) {
        ...
        _glfwInputJoystickChange(js - _glfw.linux_js.js, GLFW_DISCONNECTED);
      }
...
GLFWbool _glfwInitJoysticksLinux(void) {
  ...
  if (_glfw.linux_js.inotify == -1) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
                    "Linux: Failed to initialize inotify: %s", strerror(errno));
    return GLFW_FALSE;
  }
  ...
  _glfw.linux_js.watch =
      inotify_add_watch(_glfw.linux_js.inotify, dirname, IN_CREATE | IN_ATTRIB);
